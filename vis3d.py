...
def visualize_lp3d_polytope(
    ...
    poly = Poly3DCollection(
        faces, alpha=0.25, facecolor="tab:blue", edgecolor="k", linewidths=0.3
    )
    ...
    if x_star is not None:
        ...
        label = (
            "x* (из lpgen, допустима)" if inside_flag else "x* (из lpgen, вне области)"
        )
        ax.scatter(xs[0], xs[1], xs[2], s=80, color=color, marker="o", label=label)
